...
def main(opts):
    ...
    for model_name in model_names:
        ...
        if 'meta' in opts.mode.lower():
            ...
            model = EDVR_arch.UPEDVR(nf=nf, nframes=N_in, groups=8, front_RBs=5, 
                                     center=None, back_RBs=10, w_TSA=w_TSA, down_scale=True, 
                                     align_target=True, ret_valid=True)
        elif opts.mode.lower() == 'upcont1':
            model = EDVR_arch.UPControlEDVR(nf=nf, nframes=N_in, groups=8, front_RBs=5, 
                                            center=None, back_RBs=10, w_TSA=w_TSA, 
                                            down_scale=True, align_target=True, 
                                            ret_valid=True, multi_scale_cont=False)
        elif opts.mode.lower() == 'upcont3':
            model = EDVR_arch.UPControlEDVR(nf=nf, nframes=N_in, groups=8, front_RBs=5, 
                                            center=None, back_RBs=10, w_TSA=w_TSA, 
                                            down_scale=True, align_target=True, 
                                            ret_valid=True, multi_scale_cont=True)
        ...
        for sub in test_subs:
            ...
            for i in range(0, max_idx, n_gpus):
                ...
                for m in range(i, end):
                    ...
                    logger.info('{:3d} - {:25} \tPSNR: {:.6f} dB    SSIM: {:.6}'.format(
                        m + 1, img_names[m], crt_psnr, crt_ssim))
            ...
            logger.info('Folder {} - Average PSNR: {:.6f} dB for {} frames; '
                        ...
                        'Border PSNR: {:.6f} dB for {} frames.'.
                        format(sub, avg_psnr, (N_center + N_border), 
                            avg_psnr_center, N_center, avg_psnr_border, N_border))
            logger.info('Folder {} - Average SSIM: {:.6f} for {} frames; '
                        'Center SSIM: {:.6f} for {} frames; '
                        'Border SSIM: {:.6f} for {} frames.'.
                        format(sub, avg_ssim, (N_center + N_border), 
        ...
        for sub_name, psnr, psnr_center, psnr_border, ssim, ssim_center, ssim_border in zip(
                evaled_subs, avg_psnrs, avg_psnr_centers, avg_psnr_borders, avg_ssims, 
                avg_ssim_centers, avg_ssim_borders):
            logger.info('Folder {} - Average PSNR: {:.6f} dB. '
                        'Center PSNR: {:.6f} dB. Border PSNR: {:.6f} dB.'.
                        format(sub_name, psnr, psnr_center, psnr_border))
            logger.info('Folder {} - Average SSIM: {:.6f} '
                        'Center SSIM: {:.6f} Border SSIM: {:.6f} '.
                        format(sub_name, ssim, ssim_center, ssim_border))
...
def parse_args():
    ...
    parser.add_argument('--mode',       dest='mode',       type=str,  default='edvr')
    ...
    parser.add_argument('--model_dir',  dest='model_dir',  type=str,  
                                        default='../snapshots/REDS/upcont3/1800000_G.pth')

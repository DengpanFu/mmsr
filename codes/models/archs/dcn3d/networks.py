...
class ResP3DBottleBlock(nn.Module):
    def __init__(self, planes, if_bias=False, if_sn=False, padding_mode="const", if_relu=False, conv_type='deform', addition_back_pad=False):
        super(ResP3DBottleBlock, self).__init__()
        # self.relu = nn.ReLU(inplace=True)
        self.if_relu = if_relu
        self.conv_type = conv_type
        self.addition_back_pad = addition_back_pad
        # padding_mode = "shuyang"
        
        conv1 = nn.Conv3d(planes//4,planes//4,kernel_size=(1,3,3),stride=(1, 1, 1),padding=(0, 1, 1),bias=if_bias)
        bn1 = nn.BatchNorm3d(planes//4)
        relu1 = nn.ReLU(inplace=True)
        pad1 = nn.ReplicationPad3d((0,0,0,0,1,1))
        if self.conv_type == "deformv2":
            conv1_2 = DeformConv3dPack_v2(planes,planes//4,kernel_size=(3,1,1),stride=(1, 1, 1),padding=(0, 0, 0),bias=if_bias)
        elif self.conv_type == "deform":
            conv1_2 = DeformConv3dPack(planes,planes//4,kernel_size=(3,1,1),stride=(1, 1, 1),padding=(0, 0, 0),bias=if_bias)
        elif self.conv_type == "normal":
            conv1_2 = nn.Conv3d(planes,planes//4,kernel_size=(3,1,1),stride=(1, 1, 1),padding=(0, 0, 0),bias=if_bias)
        else:
            print("not this conv_type ")

        bn2 = nn.BatchNorm3d(planes//4)
        relu = nn.ReLU(inplace=True)
        conv3 = nn.Conv3d(planes//4, planes, kernel_size=1, stride=1, padding=0, bias=False)
        bn3 = nn.BatchNorm3d(planes)
        

        model = [pad1,conv1_2,bn2,relu1,conv1,bn1,relu,conv3,bn3]
...
class ResP3DBlock(nn.Module):
    def __init__(self, planes, if_bias=False, if_sn=False, padding_mode="const", if_relu=False, conv_type='deform', addition_back_pad=False):
        super(ResP3DBlock, self).__init__()
        # self.relu = nn.ReLU(inplace=True)
        self.if_relu = if_relu
        self.conv_type = conv_type
        self.addition_back_pad = addition_back_pad
        ...
        pad1 = nn.ReplicationPad3d((0,0,0,0,1,1))
        if self.conv_type == "deformv2":
            conv1_2 = DeformConv3dPack_v2(planes,planes,kernel_size=(3,1,1),stride=(1, 1, 1),padding=(0, 0, 0),bias=if_bias)
        elif self.conv_type == "deform":
            conv1_2 = DeformConv3dPack(planes,planes,kernel_size=(3,1,1),stride=(1, 1, 1),padding=(0, 0, 0),bias=if_bias)
        elif self.conv_type == "normal":
            conv1_2 = nn.Conv3d(planes,planes,kernel_size=(3,1,1),stride=(1, 1, 1),padding=(0, 0, 0),bias=if_bias)
        else:
            print("not this conv_type ")
        ...
        model = [conv1,relu1,pad1,conv1_2,bn1,relu,pad2,conv2,bn2]
...
class SuperNet(nn.Module):
    def __init__(self,conv_type,image_size,block,layer_num,input_channel=3, if_sn=False, padding_mode="const"):
        ...
        layer2.append(Res3DBlock(64,if_bias=if_bias, if_sn=if_sn, padding_mode=self.padding_mode, if_relu=False, conv_type=conv_type))

...
def check_dconv3d_zero_offset():
    ...
    dcn = DeformConv3d(inC, outC, (kH, kW),
                   stride=1, padding=1, dilation=1,
                   groups=2, 
                   deformable_groups=deformable_groups, im2col_step=1).cuda()
    pcn = nn.Conv2d(inC, outC, (kH, kW), stride=1, padding=1, dilation=1, groups=2).cuda()
...
def check_dconv3d_im2col_step_forward():
    ...
    dcn1 = DeformConv3d(inC, outC, (kH, kW), 
        stride=1, padding=1, dilation=1, 
        groups=groups, 
        deformable_groups=deformable_groups,
        im2col_step=1).cuda()

    dcn2 = DeformConv3d(inC, outC, (kH, kW), 
        stride=1, padding=1, dilation=1, 
        groups=groups, 
        deformable_groups=deformable_groups,
        im2col_step=2).cuda()
...
def example_dconv3d():
    input = torch.randn(2, 64, 128, 128).cuda()
    # wrap all things (offset and mask) in DCN
    dcn = DeformConv3dPack(64, 128, kernel_size=(3, 3), stride=1,
              padding=1, deformable_groups=2).cuda()
    # print(dcn.weight.shape, input.shape)
    output = dcn(input)
    targert = output.new(*output.size())
...
def example_ten_mdconv():
    input = torch.randn(1, 64, 128, 128).cuda()
    # wrap all things (offset and mask) in DCN
    start1 = time.time()

    dcn1 = ModulatedDeformConvPack(64, 64, kernel_size=(3, 3), stride=1,
              padding=1, deformable_groups=2).cuda()
    dcn2 = ModulatedDeformConvPack(64, 64, kernel_size=(3, 3), stride=1,
              padding=1, deformable_groups=2).cuda()
    dcn3 = ModulatedDeformConvPack(64, 64, kernel_size=(3, 3), stride=1,
              padding=1, deformable_groups=2).cuda()
    # print(dcn.weight.shape, input.shape)
    stop1 = time.time()
    ...
    targert = output3.new(*output3.size())
    targert.data.uniform_(-0.01, 0.01)
    error = (targert - output3).mean()
    error.backward()
